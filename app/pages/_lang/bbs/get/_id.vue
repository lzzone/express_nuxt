...
<template>
  <div class="bbs">
    <mu-appbar style="width: 100%;" color="primary" :title="bbs.title">
      ...
      <mu-button slot="right" flat color="primary" v-if="bbs.user==$store.state.user.id"  >
        公开
      </mu-button>


      <mu-button icon slot="right" v-if="bbs.user==$store.state.user.id" :to="$i18n.path('bbs/add?id=')+bbs.id">
...
export default {
    ...
    async asyncData({ app, params }) {
        ...
        var bbs_reply_res = await app.$axios.$get(
            `/api/v1/reply/get/${params.id}`
        );
        console.log(bbs_res);
        return {
